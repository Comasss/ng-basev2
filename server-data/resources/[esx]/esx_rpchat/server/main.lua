...
RegisterCommand('msg', function(source, args, rawCommand)
	...
	if tonumber(args[1]) and args[2] then
		...
		TriggerClientEvent('chat:addMessage', source, { args = { '^3[ZRX]', ' No estás usuando el comando de la forma adecuada.' } })
	end
end, false)
...
RegisterCommand('pid', function(source, args, rawCommand)
    ...
    local name = GetPlayerName(source)
    if Config.DisableESXIdentity then name = GetCharacterName(source) end
    TriggerClientEvent('chat:addMessage', source, { args = { _U('id_prefix', source, name), args }, color = { 110, 178, 159 } })
end, false)
