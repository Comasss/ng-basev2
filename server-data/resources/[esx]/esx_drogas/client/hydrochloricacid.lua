	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				ESX.ShowFloatingHelpNotification("Pulsa ~r~[E]~s~ para recoger ácido clorhídrico del bote", vector3(coords.x, coords.y, coords.z + 1))
			end
...
function PickUpHydrochloricAcid(playerPed, coords, nearbyObject, nearbyID)
	...
	ESX.TriggerServerCallback('esx_yisus_drogas:canPickUp', function(canPickUp)
		...
		if canPickUp then
			...
			TriggerServerEvent('esx_yisus_drogas:pickedUpHydrochloricAcid')
			Citizen.Wait(5000)
