	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.ChemicalsConvertionMenu.coords, true) < 2 then
			if not menuOpen then
				ESX.ShowFloatingHelpNotification("Pulsa ~r~[E]~s~ para abrir el menu de conversión química",vector3(coords.x, coords.y, coords.z + 1))
...
function OpenChemicalsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'ChemicalsConvertionMenu', {
		...
		menu.close()
		menuOpen = false
		TriggerServerEvent('esx_yisus_drogas:ChemicalsConvertionMenu', data.current.name, 1)
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				ESX.ShowFloatingHelpNotification("Pulsa ~r~[E]~s~ para coger productos quimicos", vector3(coords.x, coords.y, coords.z + 1))
			end
...
function PickUpChemicals(playerPed, coords, nearbyObject, nearbyID)
	...
	ESX.TriggerServerCallback('esx_yisus_drogas:canPickUp', function(canPickUp)
		...
		if canPickUp then
			...
			TriggerServerEvent('esx_yisus_drogas:pickedUpChemicals')
			Citizen.Wait(5000)
