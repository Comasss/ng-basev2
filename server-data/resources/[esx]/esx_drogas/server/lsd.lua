...
RegisterServerEvent('esx_yisus_drogas:processLSD')
AddEventHandler('esx_yisus_drogas:processLSD', function()
...
--		print(('esx_yisus_drogas: %s attempted to exploit lsd processing!'):format(GetPlayerIdentifiers(source)[1]))
	end
end)

RegisterServerEvent('esx_yisus_drogas:processThionylChloride')
AddEventHandler('esx_yisus_drogas:processThionylChloride', function()
	if not playersProcessingLSD[source] then
		local _source = source
...
--		print(('esx_yisus_drogas: %s intento de explotar el proceso de LSD!'):format(GetPlayerIdentifiers(source)[1]))
	end
end)
