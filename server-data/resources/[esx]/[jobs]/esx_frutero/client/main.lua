...
AddEventHandler('frutero:hasEnteredMarker', function(zone)
    if zone == 'VehicleSpawner' then
        CurrentAction = 'vehicle_spawner'
       -- CurrentActionMsg = 'Presiona ~INPUT_CONTEXT~ para sacar tu vehículo de trabajo'
        CurrentActionData = {}
        ...
        if ESX.PlayerData.job and ESX.PlayerData.job.name == 'frutero' and
            ...
            if distance < Config.DrawDistance then
                ...
                if distance < Config.Zones.Cloakroom.Size.x then
                    ...
                    ESX.ShowFloatingHelpNotification('Pulsa ~r~[E]~s~ para abrir el vestuario', vector3(405.53994750977, 6526.32421875, 26.80 + 1.5))
                end
...
function WorkJob1()
    if onDuty == true then
       ...
       --     exports['mythic_notify']:SendAlert('error', "Deja el vehículo en el camino, estas destrozando la huerta")
            ESX.ShowNotification("Deja el vehículo en el camino, estas destrozando la huerta")
            Citizen.Wait(2000)
            HasAlreadyEnteredMarker, LastZone = true, currentZone
            TriggerEvent('frutero:hasEnteredMarker', currentZone)
        else
           ...
           ESX.ShowNotification("No puedes hacer el trabajo montado en un vehículo")
                Citizen.Wait(2000)
                HasAlreadyEnteredMarker, LastZone = true, currentZone
                TriggerEvent('frutero:hasEnteredMarker', currentZone)
            else
               ...
               ESX.ShowNotification("Primero, coloca la fruta en tu vehículo")
                    else
                        animRecolectando()
                    ...
                    ESX.ShowNotification("Pon la fruta en el vehículo de trabajo")
                        Citizen.Wait(3000)
                        HasAlreadyEnteredMarker, LastZone = true, currentZone
                        TriggerEvent('frutero:hasEnteredMarker', currentZone)
                    end
                 ...
                 ESX.ShowNotification("Tienes que acercarte al árbol para recoger algo de él")
                    Citizen.Wait(2000)
                    HasAlreadyEnteredMarker, LastZone = true, currentZone
                    TriggerEvent('frutero:hasEnteredMarker', currentZone)
                end
    ...
    if onDuty == true then
        if not ESX.Game.IsSpawnPointClear(Config.Zones.Job1.Pos, 20.0) then
         --   exports['mythic_notify']:SendAlert('error', "¡Alguien dejó el vehículo en el huerto...!")
            ...
            if IsPedInAnyVehicle(PlayerPedId()) then
                ESX.ShowHelpNotification("~r~¡No puedes hacer este trabajo mientras estás en la carretera!")
                Citizen.Wait(2000)
                HasAlreadyEnteredMarker, LastZone = true, currentZone
                TriggerEvent('frutero:hasEnteredMarker', currentZone)
            else
                ...
                if tree ~= 0 then
                    if isHoldingFruit == true then
              --          exports['mythic_notify']:SendAlert('error', "Primero, coloca la fruta en tu vehículo")
              EXX.ShowNotification("Primero, coloca la fruta en tu vehículo")
                    else
                        animRecolectando()
                        FreezeEntityPosition(PlayerPedId(), true)
                        porcentaje(150)
...
function WorkJob3a()
    ...
    if IsPedInAnyVehicle(PlayerPedId(), false) and IsInAuthorizedVehicle() then
     ...
     ESX.ShowNotification("Lleva la fruta a la tienda y vendela")
    else
...
AddEventHandler('frutero:toomuch', function()
  ...
  ESX.ShowNotification("Llevas demasiada fruta, no seas avaricioso.")
    ClearPedTasks(PlayerPedId())
end)
...
AddEventHandler('frutero:insuficiente', function()
  ...
  ESX.ShowNotification("No tienes la cantidad necesaria de fruta.")
    ClearPedSecondaryTask(PlayerPedId())
end)
...
AddEventHandler('frutero:darcajafruta', function()
    ...
    while (darzumo) do
        ...
        local distance = GetDistanceBetweenCoords(coordsy.x, coordsy.y, coordsy.z, 1732.67, 6421.91, 34.14, true)
        if distance < 1.5 then
            ESX.ShowFloatingHelpNotification("Pulsa ~r~[E]~s~ para vender fruta", vector3(1732.67, 6421.91, 34.14 + 1.3))
        end
...
function onduty()
    if not BlipsAdded then
        ...
        SetBlipColour(blipFruterod2, 83)
