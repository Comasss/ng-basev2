...
RegisterServerEvent('esx_ambulancejob:curationscript')
AddEventHandler('esx_ambulancejob:curationscript',function()
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'ambulance' then
			enfermeros = enfermeros+1
		end
	end
	if (price <= xPlayer.getAccount('bank').money) and enfermeros == 0 then
		xPlayer.removeAccountMoney('bank', price)
		xPlayer.triggerEvent('esx_ambulancejob:revive')
	else
		TriggerClientEvent('esx:showNotification', source, '~r~Necesitas 2500 para pagar tus heridas y verifica que ya no haya enfermeros en la ciudad')
	end
end)